...
Recipe getWeightedRecipe(Recipe *recipes, int amount_of_recipes, Recipe *planned_recipes, int amount_of_already_planned){
    ...
    for(int i = 0; i < amount_unused; i++){
        WeightedRandom = get_random_number(0, amount_unused + matches[i]) + matches[i]; /*This generates a random number between zero and amount_unused + whatever the corrospondings recipe has "scored"*/
